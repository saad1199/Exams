      ...
      <div class="content-inner w-100" style="background-image:url('assets/img/curved-images/white-curved.jpg');">
        ...
        <header class="bg-blue text-white shadow-sm px-4 py-3 z-index-20">
          <div class="container-fluid px-0">
            <h2 class="mb-0 p-1">Complaint Management System</h2>
          </div>
        </header>
        <!-- Page Header-->
            ...
            <div class="col-lg-12">
                <div class="card mb-0">
                  <div class="card-header  bg-dark text-white">
                    ...
                    <h3 class="h4 mb-0 "> <img src="img/govw.png" width="40">&nbsp;&nbsp;&nbsp;Inititate Complain</h3><br><br>
                      </div>
                  <div class="card-body" id="list-items">
                  
                  <form class="form-horizontal" method="post" action="Engine/rs_submit_complaint.php" enctype="multipart/form-data">
                      <div class="row">
                        <label class="col-sm-3 form-label">Complaint Description</label>
                        <div class="col-sm-9">
                          <textarea required class="form-control" type="textarea" name="complain-text" id="complain-text" style="height:300px;"></textarea>
                        </div>
                      </div>
                      <br>
                      <div class="row">
                        <label class="col-sm-3 form-label">Complainant Name (your name)</label>
                        <div class="col-sm-9">
                          <input required class="form-control" type="text" name="complain-name" id="complain-name" />
                        </div>
                      </div>
                      <br>
                      <div class="row">
                        <label class="col-sm-3 form-label">Institute Contact Number</label>
                        <div class="col-sm-9">
                          <input required class="form-control" type="text" name="complain-phone" id="complain-phone" />
                        </div>
                      </div>
<br>
                      <div class="row" style="text-align:right;" >
                        <div class="col-sm-12">
                           <input class="btn btn-danger text-white w-40" name="submit-complain" type="submit" value="Inititate">   
                      </div>
                      </div>
...
xcode+="<thead><th>#</th><th>Date Initiated</th><th>Complain Description</th><th>Admin Reply</th><th>Status</th></thead>"
      xcode += '<tbody>';
      ...
      for (var i = 0; i < myJson.length; i++) {
        ...
        xcode += '<td>'+(i+1)+'</td>';
        xcode += '<td>'+myJson[i].complain_date+'</td>';
        xcode += '<td><textarea readonly>'+myJson[i].complain_desc+'</textarea></td>';
        
        if(myJson[i].complain_reply==null){
          xcode += '<td><textarea readonly>-</textarea></td>';

        }else{
          xcode += '<td>'+myJson[i].complain_reply+'</td>';

        }
        if(myJson[i].complain_status==0){
          xcode += '<td class="bg-info text-white">Pending</td>';
        }else if (myJson[i].complain_status==1){
          xcode += '<td class="bg-success text-white">Resolved</td>';
        }
...
<script>
  ...
  if (status) {
    if (status == "ERROR") {
      ...
      document.getElementById("danger-alert").innerHTML = "<strong>ERROR !</strong> Cannot INITIATE COMPLAIN Something went wrong LOG REPORTED- Please Report IT Section Immediately";
    }
    if (status == "SECURITY_ERROR") {
      document.getElementById("danger-alert").style.display = "block";
      document.getElementById("danger-alert").innerHTML = "<strong>SECURITY ERROR !</strong> Cannot INITIATE COMPLAIN Something went wrong LOG REPORTED- Please Report IT Section Immediately";
    }
    if (status == "OK") {
      document.getElementById('success-alert').style.display = "block";
      document.getElementById('success-alert').innerHTML = "<strong>SUCCESS !</strong>  COMPLAIN has Successfully Initiated";
    }
    if (status == "ALREADY") {
      document.getElementById("danger-alert").style.display = "block";
      document.getElementById("danger-alert").innerHTML = "<strong>ALREADY SUBMITTED !</strong> COMPLAIN ALREADY POSTED to FDE HQ - Please wait for response ";
