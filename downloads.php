...
$connx = OpenCon();
$status = getInstituteLockStatus($_SESSION['inst_code'],$connx);


if ($status == '0') {
header('location:index.php?status=CHALLAN_NOT_SUBMITTED');
exit();
}
...
<body class="bg-gray-100">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    <div class="container-fluid ">
      <div class="container-fluid">
<div><h3>Please use PRINT Button at the bottom of this page to PRINT . If your students are missing in the list their challan is not submitted to FDE / Payment is not cleared</h3></div>
        <div class="pagebr" id="report">
          ...
          if ($live_key == $session_token) {
            ...
            if(isInstituteUpdated($conn,$code)){
              if(getInstituteType($conn,$code)=="FDE"){
                $count = getTeacherCount($conn,$code);
                if($count>0){
                  $grantAccess = true;
                  $bit=true;
                }
          ...
          if ($grantAccess) {
            ...
            LEFT JOIN proposedInstitute ON StudentRegistration.reg_allocated_center_inst_id = proposedInstitute.pins_id LEFT JOIN BankChallan ON StudentRegistration.reg_challan_detail_challan_id=BankChallan.challan_id WHERE Student.student_institute_inst_id='" . $code . "' AND isPaid=1 ORDER BY reg_roll_no ASC;";
                      ...
                      <table class="w-100">
                       ...
                       if(is_null($row['student_institute_inst_id'])){
                           ...
                           $instid=$row['student_institute_inst_id'];
                                                       ...
                                                       $path = "uploads-new/".$instid."/".$row['student_img'];
                                                       //$path_jpeg = "uploads/" .$bform. "-" . $row['student_name'] . ".jpeg";
                                                       if (file_exists($path)) {
                                                         echo '<img  width="110" height="110" src="'.$path.'"      class="">';
                                                       }else {
          ...
          <?php
            ...
            if($bit==false){
              echo '<h2 class="text-danger"><br><br>No ACCESS TO THIS PAGE : Please update teachers data (ONLY FOR FDE) by  <a href="/requirement_form.php">clicking here</a> and Institute Contact Details (FOR ALL) by  <a href="/admin-profile.php">clicking here</a> or from the dashboard first</h2>';
            }else{
