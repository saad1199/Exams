<?php session_start();
$sequence = array(83,121,115,116,101,109,32,100,101,118,101,108,111,112,101,100,32,98,121,32,83,97,97,100,32,83,97,100,105,113,32,40,83,121,115,116,101,109,32,83,111,102,116,119,97,114,101,32,69,110,103,105,110,101,101,114,41,32,45,32,73,84,32,70,68,69); 
...
<body class="bg-gray-100">
  ...
  //           $sql = "SELECT `Student`.*, `StudentRegistration`.*, RefGender.*, RefReligion.*, RefStudentType.*,SubjectCategory.*,proposedInstitute.* FROM `Student` LEFT JOIN `StudentRegistration` ON `StudentRegistration`.`student_student_id` = `Student`.`student_id` LEFT JOIN RefGender ON Student.student_gender_gender_id=RefGender.gender_id LEFT JOIN RefReligion ON Student.student_religion_religion_id=RefReligion.religion_id LEFT JOIN RefStudentType ON StudentRegistration.reg_student_type_std_type_id = RefStudentType.std_type_id LEFT JOIN SubjectCategory ON StudentRegistration.reg_student_subjectCat_sub_cat_id=SubjectCategory.sub_cat_id
  // LEFT JOIN proposedInstitute ON StudentRegistration.reg_allocated_center_inst_id = proposedInstitute.pins_id WHERE Student.student_id='".$code."';";
            
            $sql = "SELECT `Student`.*, `StudentRegistration`.*, RefGender.*, RefReligion.*, RefStudentType.*,SubjectCategory.*,proposedInstitute.*,BankChallan.* FROM `Student`  LEFT JOIN `StudentRegistration` ON `StudentRegistration`.`student_student_id` = `Student`.`student_id` LEFT JOIN RefGender ON Student.student_gender_gender_id=RefGender.gender_id LEFT JOIN RefReligion ON Student.student_religion_religion_id=RefReligion.religion_id LEFT JOIN RefStudentType ON StudentRegistration.reg_student_type_std_type_id = RefStudentType.std_type_id LEFT JOIN SubjectCategory ON StudentRegistration.reg_student_subjectCat_sub_cat_id=SubjectCategory.sub_cat_id
            LEFT JOIN proposedInstitute ON StudentRegistration.reg_allocated_center_inst_id = proposedInstitute.pins_id LEFT JOIN BankChallan ON StudentRegistration.reg_challan_detail_challan_id=BankChallan.challan_id WHERE isPaid=1 AND Student.student_id='".$code."';";
          ...
          ?>
              ...
              <div class="card card-body blur shadow-blur mx-4">
                  <div class="row">
                    ...
                    <h5 style="font-size:14px;"><strong><u>CENTRALIZED ANNUAL EXAMINATION 2024</u></strong></h5>
                      </div>
                  </div>
              </div>
                    ...
                    <div class="w-100">
                      <table class="w-100">
                       ...
                       $bform= $row['student_b_form'];
                                                      $path = "uploads-new/000/".$row['student_img'];
                                                
                                                      if (file_exists($path)) {
                                                        echo '<img  width="110" height="110" src="uploads-new/000/'.$row['student_img'].'" class="">';
                                                      } else {
                        ...
                        <th class="w-50 text-right">
                          <h5 id="disp-name" class="text-s">Roll # <strong> <?php echo $row['reg_roll_no']; ?></strong></h5> 
                ...
                <div class="container-fluid py-1">
                          ...
                          <div class="">
                            <table class="wrapping-table h-100 mb-0">
                              <tbody id="basic-table-body">
                                ...
                                <tr>
                                  <td class="text-uppercase text-secondary text-xs font-weight-bolder opacity-10 ps-2  text-dark w-30">Registration #</td>
                                  <td style="font-size:14px;"><?php echo '051'.$row['student_id']; ?></td>
                                </tr>
                                <tr>
                  ...
                  <div>
                  <p class="text-xxs"><strong>Note:- </strong>This Roll Number Slip is computer generated and hence doesn't need any signature <strong><span name="notes" style="opacity:0.9; font-style:italic;  font-size:9px;"></span></strong></p>
                  </div>
                </div>
              </div>
          ...
          <?php
            ...
            if($isOK==false){
              echo '<br><h4 class="text-danger"><strong class="bg-dark text-light">Payment Status not cleared</strong> - <br>If you have submitted challan already then, please visit EXAM Section FDE along with the paid challan copy.</h4>';
            }
            mysqli_close($conn);
          } else {
        ...
        <div style="text-align:center;">
          ...
          <h6>Save it as PDF as well it will be helpful later on. This page is not compatible/suitable for mobile webview due to print layout. Use Desktop Browser for printing</h6>
